    ...
    while (std::getline(*serverContent, line))
    {
        ...
        if (fword == "listen")
        {
            ...
            if (find == std::string::npos)
            {
                ...
                server.s_sin_port = htons(ft_stoi(server.getPort_s()));
                server.s_host = "127.0.0.1";
                server.s_addr = htonl(convert_str_to_uint32("127.0.0.1"));
            }
            else
            {
                ...
                server.s_addr = htonl(convert_str_to_uint32(server.s_host));
            }
        }
